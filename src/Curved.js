...
const balance_of_time = (unspends, spents) => {
  ...
  let balance_of_time = new Map();

  _.reduce(
    actions_of_time,
    (cur_unspends, actions) => {
      ...
      cur_unspends = cur_unspends.concat(new_unspends);

      // console.log(cur_unspends, 'cur_unspends')

      // sub spents
      _.filter(actions, x => x.type === "spent").forEach(x => {
        cur_unspends = _.filter(
          cur_unspends,
          u => u.txid !== x.txid || u.index !== x.index
        );
      });
      // console.log(cur_unspends, "unspents");
      let cur_balance = 0;
      _.each(cur_unspends, u => (cur_balance += u.v));
      // console.log(cur_balance, 'balance');
      balance_of_time.set(t, cur_balance);
      return cur_unspends;
    },
    []
  );
